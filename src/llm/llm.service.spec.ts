...
import {
  ConversationPort,
  CONVERSATION_PORT,
} from '../conversation/conversation.port';
...
describe('LlmService', () => {
  ...
  beforeEach(async () => {
    ...
    mockInMemoryCacheService = {
      ...
      getOrCreateSystemPrompt: jest
        .fn()
        .mockImplementation((_, generator) => generator()),
      appendToConversation: jest.fn(),
    };
    mockGeminiCacheService = {
      getOrCreateSystemPromptCache: jest
        .fn()
        .mockResolvedValue('test-cache-name'),
    };
    mockUserService = {
      getUserPersonalityPreset: jest
        .fn()
        .mockResolvedValue(toPersonalityPresetId('test_preset')),
    };
  ...
  it('should fallback to default preset if user preset not found', async () => {
    ...
    mockPersonalityPresetService.findById = jest
      .fn()
    ...
    expect(mockPersonalityPresetService.findById).toHaveBeenCalledWith(
      'default_assistant',
    );
    expect(mockFileSearchAssistant.answerQuestion).toHaveBeenCalledWith(
      'Hello',
      expect.objectContaining({
        systemInstruction: expect.stringContaining(
          'プリセット ID: test_preset',
        ), // default mock returns test_preset
      }),
    );
  });
  ...
  it('should use caches correctly', async () => {
    ...
    expect(
      mockInMemoryCacheService.getOrCreateConversation,
    ...
    expect(
      mockGeminiCacheService.getOrCreateSystemPromptCache,
    ).toHaveBeenCalled();

    // メッセージがキャッシュに追加されているか確認
    expect(mockInMemoryCacheService.appendToConversation).toHaveBeenCalledTimes(
      2,
