...
import {
  ...
  PersonalityPresetProps,
} from '../personalityPreset.types';

@Injectable()
export class JsonPersonalityPresetRepository
  implements PersonalityPresetRepository, OnModuleInit
{
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        if (fs.existsSync(srcPath)) {
          ...
          const rawPresets: (Omit<PersonalityPresetProps, 'id'> & {
            id: string;
          })[] = JSON.parse(fileContent);
          // string型のidをPersonalityPresetId型に変換
          this.presets = rawPresets.map((raw) =>
            PersonalityPreset.create({
              ...raw,
              id: raw.id as PersonalityPresetId,
            }),
          );
          this.logger.log(
            `Loaded ${this.presets.length} personality presets from ${srcPath}`,
          );
          return;
        }
        this.logger.error(
          `Presets file not found at ${filePath} or ${srcPath}`,
        );
        return;
      }
      ...
      this.logger.log(
        `Loaded ${this.presets.length} personality presets from ${filePath}`,
      );
    } catch (error) {
